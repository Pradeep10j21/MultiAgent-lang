...
export default function SideBar({ handleClick, thread }: SidebarProps){
    ...
    const renderChatHistory = chatHistory.map(chat => <button key={chat.thread_id} className={`text-left cursor-pointer px-4 hover:bg-white/10 duration-200 rounded-lg py-2 truncate ${applyHighlight(chat.thread_id)}`} onClick={() => handleClick(chat.thread_id)}>{chat.title}</button>)
    ...
    return (
        <div className="bg-white/10 p-2 border border-white/20 shadow-lg w-64 h-[95vh] m-4 rounded-xl flex flex-col justify-between">
            ...
            <button onClick={() => handleClick(null)} className="bg-white/10 text-white/80 cursor-pointer hover:bg-white/20 duration-200 text-sm rounded-lg py-2 flex items-center justify-center gap-1">
