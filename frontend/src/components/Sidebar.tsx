...
export default function SideBar({ handleClick, thread }: SidebarProps){
    ...
    const renderChatHistory = chatHistory.map(chat => <button key={chat.thread_id} className={`text-left cursor-pointer px-4 hover:bg-white/10 duration-200 rounded-lg py-2 truncate ${applyHighlight(chat.thread_id)}`} onClick={e => handleClick(chat.thread_id)}>{chat.title}</button>)
